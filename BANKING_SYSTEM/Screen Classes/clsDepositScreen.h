...
public:
	static void Deposit(string File_Name = "Data/Clients.txt")
	{
		...
		if (clsBankClient::IsClientExist(CCP_Number))
		{
			...
			for (clsBankClient Client : V1)
			{
				...
				if (Client.CCP_Number() == CCP_Number)
				{
					...
					do
					{
						...
						cout << setw(37) << left << "" << "Do You Confirm Deposing In This Account  Y/N \n";
					...
					else
					{
						...
						int Amount_To_Add;
						Amount_To_Add = clsInputValidation::ReadIntNumber();

						Client.Balance += Amount_To_Add;
